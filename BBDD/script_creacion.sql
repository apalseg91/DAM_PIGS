...
CREATE TABLESPACE FITMANAGE
DATAFILE 'fitmanage01.dbf'
SIZE 100M
AUTOEXTEND ON NEXT 10M MAXSIZE UNLIMITED;

CREATE PROFILE FITMANAGE LIMIT PASSWORD LIFE_TIME UNLIMITED;
...
BEGIN
    ...
    WHERE c.ID_CLIENTE IN (
        SELECT p.ID_CLIENTE
        FROM PAGO p
        WHERE UPPER(p.CONCEPTO) = 'CUOTA MENSUAL')
    AND c.ACTIVO = 0;
    UPDATE CLIENTE c
    SET FECHA_PROXIMO_PAGO =
        ADD_MONTHS(
            GREATEST(NVL(c.FECHA_PROXIMO_PAGO, SYSDATE),SYSDATE),1)
    WHERE c.ID_CLIENTE IN (
        SELECT p.ID_CLIENTE
        FROM PAGO p
        WHERE UPPER(p.CONCEPTO) = 'CUOTA MENSUAL')
